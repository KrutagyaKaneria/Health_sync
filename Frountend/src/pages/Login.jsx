...
const Login = () => {
  ...
  const submitHandler = async (event) => {
    ...
    try {
      ...
      // ✅ Ensure token and userId are present before storing
      if (result.token) {
  ...
  // ✅ Handle Google Login (Store Token if Backend Supports It)
  const handleGoogleLogin = async () => {
    try {
      ...
      // 🔥 If your backend supports Firebase authentication, send user token to backend
      const idToken = await user.getIdToken();

      localStorage.setItem('firebaseToken', idToken); // Store Firebase Auth token
  ...
  // ✅ Handle Facebook Login (Store Token if Backend Supports It)
