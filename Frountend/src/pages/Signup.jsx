...
const Signup = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    gender: "",
    role: "patient",
    licenseNumber: "", // Added for driver
    ambulanceNumber: "", // Added for driver
  });
  ...
  const submitHandler = async (event) => {
    ...
    if (formData.role === "driver" && (!formData.licenseNumber || !formData.ambulanceNumber)) {
      toast.error("License Number and Ambulance Number are required for drivers!");
      setLoading(false);
      return;
    }
            ...
            <div className='rounded-l-lg lg:pl-16 py-10'>
              ...
              <form onSubmit={submitHandler}>
                ...
                {formData.role === "driver" && (
                  <>
                    <div className='mb-5'>
                      <input type="text" placeholder='License Number' name='licenseNumber' value={formData.licenseNumber} onChange={handleInputChange} className='w-full pr-4 py-3 border-b border-solid border-[#0066ff61] focus:outline-none focus:border-b-primaryColor text-[16px] leading-7 text-headingColor placeholder:text-textColor cursor-pointer' required />
                    </div>
                    <div className='mb-5'>
                      <input type="text" placeholder='Ambulance Number' name='ambulanceNumber' value={formData.ambulanceNumber} onChange={handleInputChange} className='w-full pr-4 py-3 border-b border-solid border-[#0066ff61] focus:outline-none focus:border-b-primaryColor text-[16px] leading-7 text-headingColor placeholder:text-textColor cursor-pointer' required />
                    </div>
                  </>
                )}
